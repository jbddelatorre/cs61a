...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    player_score, opponent_score = player_score%100, opponent_score%100
    def abs_diff(n):
        tens = n // 10
        ones = n % 10
        if tens > ones:
    ...
    if abs_diff(player_score) == abs_diff(opponent_score):
        return True
    else:
        return False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if player == 0:
            point_scored = take_turn(strategy0(score0,score1), score1, dice)
            score0 += point_scored
            if is_swap(score0, score1):
                score0, score1 = score1, score0
        else:
            point_scored = take_turn(strategy1(score1,score0), score0, dice)
            score1 += point_scored
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    highest_num_rolls = 0
    while num_rolls <= 10:
        check_score = make_averaged(roll_dice, num_samples)
        check = check_score(num_rolls, dice)
        if ( check > higest_average ):
            higest_average = check 
